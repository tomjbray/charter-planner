...
function renderCards(results) {
  ...
  filtered.forEach((r, idx) => {
    ...
    const regEntries = minYear
      ? allRegEntries.filter(e => e.year && e.year >= minYear)
      : allRegEntries;
...
function populateRegistry(typeCode, card) {
  ...
  // Apply year filter if set — show only aircraft built in minYear or later
  const entries = minYear
    ? allEntries.filter(e => e.year && e.year >= minYear)
    : allEntries;
  ...
  const moreHtml = remaining > 0
    ? `<div class="reg-more">+ ${remaining.toLocaleString()} more registered in the US</div>`
    : '';
